...
class MistralCommonTokenizer(PushToHubMixin):
    ...
    def apply_chat_template(
        ...
        reasoning_effort: Optional[str] = None,  # THOR BACKPORT of transformers#44760
        **kwargs,
        ...
        """
        # THOR BACKPORT of huggingface/transformers#44760 (reasoning_effort support).
        # transformers 5.x adds reasoning_effort to MistralCommonTokenizer and forwards it
        # to ChatCompletionRequest.from_openai, but transformers 5.x needs Python>=3.13 and
        # the Jetson Thor vLLM image is py3.12 — so this backports just the reasoning_effort
        # plumbing into 4.57.3. mistral_common 1.11.0 already supports the field.
        # reasoning_effort is now an explicit parameter (above), so it is not in kwargs.
        if kwargs:
            raise ValueError(
        ...
        for conversation in conversations:
            ...
            if reasoning_effort is not None:
                _from_openai_kwargs["reasoning_effort"] = reasoning_effort
            ...
            # THOR: force the reasoning turn. mistral_common 1.11.0 injects the
            # [MODEL_SETTINGS]{"reasoning_effort":"high"} block but does NOT open the
            # assistant turn with [THINK], and this NVFP4 checkpoint won't emit it on its
            # own — so reasoning never starts. Prefill [THINK] when high; the model then
            # produces the reasoning trace and closes with [/THINK] before the answer
            # (reasoning_parser="mistral" then splits it into reasoning_content).
            _toks = list(tokenized_request.tokens)
            _txt = tokenized_request.text
            if reasoning_effort == "high":
                try:
                    _think_id = self.tokenizer.instruct_tokenizer.tokenizer.get_control_token("[THINK]")
                except Exception:
                    _think_id = 34  # [THINK] rank in this model's tekken.json
                _toks = _toks + [_think_id]
                _txt = _txt + "[THINK]"
            if tokenize:
                outputs.append(_toks)
            else:
                outputs.append(_txt)
