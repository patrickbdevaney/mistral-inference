...
class MistralTokenizer(TokenizerLike):
    ...
    def apply_chat_template(
        ...
        **kwargs,
        ...
        # ---- THOR PATCH (file-mounted, see thor-decode/patches/) ----
        # The Jetson Thor image ships transformers 4.57.3 / mistral_common 1.11.0,
        # which predate reasoning_effort support: MistralCommonTokenizer.apply_chat_template
        # rejects ANY extra kwarg, so forwarding reasoning_effort (even None) 400s every
        # chat request. Only forward it if the installed transformers actually accepts it;
        # otherwise drop it so chat works in instruct mode.
        if self.version >= 15 and kwargs.get("reasoning_effort") is not None:
            import inspect as _inspect
            try:
                _sig = _inspect.signature(
                    self.transformers_tokenizer.apply_chat_template
                )
                _accepts = (
                    "reasoning_effort" in _sig.parameters
                    or any(
